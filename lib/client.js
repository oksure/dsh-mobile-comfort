window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 4. Some touch engines omit the follow-up click after touchend; a short
		 *    fallback timer activates a session row only when that click is missing.
		 */
		const CSS = [
		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				const closeDrawerAfterSessionClick = (event) => {
					...
					if (row === null) return;
					const pending = pendingTouchActivations.get(row);
					if (pending !== void 0) {
						clearTimeout(pending);
						pendingTouchActivations.delete(row);
					}
					const sidebar = openMobileSession(row);
					if (sidebar === null) return;
					sidebar.querySelector("button[aria-label=\"Collapse sidebar\"]")?.click();
				};
				const activateSessionAfterTouch = (event) => {
					if (!coarse.matches) return;
					const row = sessionRowFromTarget(event.target);
					if (row === null || openMobileSession(row) === null) return;
					const previous = pendingTouchActivations.get(row);
					if (previous !== void 0) clearTimeout(previous);
					const timer = setTimeout(() => {
						pendingTouchActivations.delete(row);
						if (row.isConnected && openMobileSession(row) !== null) row.click();
					}, 350);
					pendingTouchActivations.set(row, timer);
				};
				...
				document.addEventListener("touchend", activateSessionAfterTouch);
				return () => {
					...
					document.removeEventListener("touchend", activateSessionAfterTouch);
					for (const timer of pendingTouchActivations.values()) clearTimeout(timer);
